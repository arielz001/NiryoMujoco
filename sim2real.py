...
if robot_connected:
    try:
        # El hardware lee de forma directa y nativa sin cruzar índices
...
def send_to_txt(*args):
    """Convierte los sliders a radianes e INTERCAMBIA J4 y J5 SOLO para MuJoCo"""
    ui_rad_vals = []
    for i in range(6):
        deg = sliders[i].get()
    ...
    mujoco_joints_rad = [
        ...
        ui_rad_vals[1],
        ui_rad_vals[2],
        ui_rad_vals[4],  # El slider J5 controla el Joint de la malla 4 de tu XML
        ui_rad_vals[3],  # El slider J4 controla el Joint de la malla 5 de tu XML
        ui_rad_vals[5]
    ]
    
    # Agregar la garra al final
    g_val = slider_g.get()
    val_lbl_g.config(text=f"{g_val:.3f}")
    mujoco_joints_rad.append(g_val)
...
lbl_g = ttk.Label(frame_g, text="Garra 1", width=15)
...
def move_robot():
    """Toma los valores nativos de los sliders y mueve el robot real directamente"""
    joints_rad = [np.radians(slider.get()) for slider in sliders]
    
    if robot_connected:
        try:
            # Enviamos el orden nativo directo al robot real
            robot.move_joints(joints_rad)
            print("¡Robot Real movido exitosamente de forma nativa!")
        except Exception as e:
...
def refresh():
    """Lee la pose del hardware en orden nativo y la plasma directo en los sliders"""
    if robot_connected:
        try:
            joints_rad = robot.get_joints()
            for i in range(6):
...
# Escribe la primera línea en el TXT con el cruce aplicado para el arranque de MuJoCo
send_to_txt()
